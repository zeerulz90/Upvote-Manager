...
import { useUpvoteSelection } from '../context/UpvoteListsContext';

export default function Upvote({ list, upvoteIndex }) {
    const toggleSelection = useUpvoteSelection();
    const selected = list.selected;
    const style = {
        backgroundColor: selected ? '#E5E8FD' : '#F4F6F8',
        arrowColor: selected ? '#253CF2' : '#343A40',
    }
    return (
        <div className="upvote" style={{ backgroundColor: style.backgroundColor }} onClick={() => toggleSelection(list.id)} data-testid={"upvote-" + upvoteIndex}>
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={style.arrowColor} data-testid={"upvote-svg-" + upvoteIndex}>
